...
static void NvInitGroups(NvPrimitiveGroup * g, uint32_t count)
{
	for (uint32_t i =  0; i < count; i++)
		g[i] = NvPrimitiveGroupDef();
}
...
void NvDestroyPrimGroups(NvPrimitiveGroup ** g, unsigned int count)
{
	for (unsigned int i = 0; i < count ; i++)
		NvDestroyPrimGroup(g[i]);
	delete[] g;
}

void NvEnableRestart(const unsigned int _restartVal)
{
	bRestart = true;
...
// Default value: false
//
void NvSetListsOnly(const bool _bListsOnly)
{
...
// Default value: 16
//
void NvSetCacheSize(const unsigned int _cacheSize)
{
...
// Default value: true
//
void NvSetStitchStrips(const bool _bStitchStrips)
{
...
// Default value: 0
//
void NvSetMinStripSize(const unsigned int _minStripSize)
{
					...
					NvPrimitiveGroup** primGroups, unsigned short* numGroups, bool validateEnabled)
{
	...
	if(bListsOnly)
	{
		...
		*numGroups = 1;
		(*primGroups) = new NvPrimitiveGroup[*numGroups];
		NvInitGroups(*primGroups, *numGroups);
		NvPrimitiveGroup* primGroupArray = *primGroups;

		//count the total number of indices
		unsigned int numIndices = 0;
		for(size_t i = 0; i < tempStrips.size(); i++)
		{
			numIndices += (uint32_t)tempStrips[i]->m_faces.size() * 3;
		}

		//add in the list
		numIndices += (uint32_t)tempFaces.size() * 3;

		primGroupArray[0].type       = NV_PT_LIST;
		...
		*numGroups = numSeparateStrips; //for the strips
		...
		for(unsigned int stripCtr = 0; stripCtr < numSeparateStrips; stripCtr++)
		{
			...
			if(!bStitchStrips)
			{
				...
				stripLength = (uint32_t)stripIndices.size();
			
			primGroupArray[stripCtr].type       = NV_PT_STRIP;
		...
		if(tempFaces.size() != 0)
		{
			...
			primGroupArray[faceGroupLoc].type       = NV_PT_LIST;
			primGroupArray[faceGroupLoc].indices    = new unsigned short[tempFaces.size() * 3];
			primGroupArray[faceGroupLoc].numIndices = (uint32_t)tempFaces.size() * 3;
			size_t indexCtr = 0;
...
{
	(*remappedGroups) = new NvPrimitiveGroup[numGroups];
	NvInitGroups(*remappedGroups, numGroups);
