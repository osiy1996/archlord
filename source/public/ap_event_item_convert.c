...
struct ap_event_item_convert_module {
	...
	struct ap_module_registry * registry)
{
	...
	if (!ap_event_manager_register_event(mod->ap_event_manager,
			AP_EVENT_MANAGER_FUNCTION_ITEMCONVERT, mod,
...
static void onclose(struct ap_event_item_convert_module * mod)
{
}

static void onshutdown(struct ap_event_item_convert_module * mod)
{
}

struct ap_event_item_convert_module * ap_event_item_convert_create_module()
{
	struct ap_event_item_convert_module * mod = ap_module_instance_new(AP_EVENT_ITEM_CONVERT_MODULE_NAME,
...
boolean ap_event_item_convert_on_receive(
	...
	void * user_data)
{
	struct ap_event_item_convert_cb_receive cb = { 0 };
	...
	return ap_module_enum_callback(mod, AP_EVENT_ITEM_CONVERT_CB_RECEIVE, &cb);
}
	...
	uint32_t character_id)
{
	...
	au_packet_make_packet(&mod->packet, buffer, 
		TRUE, &length, AP_EVENT_ITEMCONVERT_PACKET_TYPE,
		&type, /* operation */
